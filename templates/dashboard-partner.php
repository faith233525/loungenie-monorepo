...
if (null === $company) {
	error_log(sprintf('LounGenie Portal: Company query failed in %s:%d - %s', __FILE__, __LINE__, $wpdb->last_error));
	$company = (object) array('name' => 'Unknown');
}
if (null === $unit_count) {
	$unit_count = 0;
}
if (null === $open_requests) {
	error_log(sprintf('LounGenie Portal: Service requests query failed in %s:%d - %s', __FILE__, __LINE__, $wpdb->last_error));
	$open_requests = 0;
}

// Get management company if exists
$management_company = null;
if (isset($company->management_company_id) && $company->management_company_id) {
	...
	// Error handling
	if (null === $management_company) {
		error_log(sprintf('LounGenie Portal: Management company query failed in %s:%d - %s', __FILE__, __LINE__, $wpdb->last_error));
		$management_company = null;
	}
}
