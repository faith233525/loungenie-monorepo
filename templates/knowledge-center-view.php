...
// Ensure training video helper is available even if loader order changes.
if (! class_exists('LGP_Training_Video')) {
	if (defined('LGP_PLUGIN_DIR')) {
		$help_guide_path     = LGP_PLUGIN_DIR . 'includes/class-lgp-knowledge-guide.php';
		$training_video_path = LGP_PLUGIN_DIR . 'includes/class-lgp-training-video.php';

		if (file_exists($help_guide_path)) {
			require_once $help_guide_path;
		}
		if (file_exists($training_video_path)) {
			require_once $training_video_path;
		}
	}
}

$categories = class_exists('LGP_Training_Video') ? LGP_Training_Video::get_categories() : array();
?>
		...
		<div class="lgp-modal">
			...
			<div class="lgp-modal-body">
				<form id="lgp-video-form">
					...
					<div class="lgp-form-group">
						<label for="lgp-video-url"><?php esc_html_e('Video URL or Upload', 'loungenie-portal'); ?> *</label>
						<input type="url" id="lgp-video-url" class="lgp-input" placeholder="https://youtube.com/watch?v=..." />
						<small class="lgp-help-text">
							<?php esc_html_e('YouTube/Vimeo/direct URL, or upload a video file below (mp4/mov/webm/ogg, max 10MB). One is required.', 'loungenie-portal'); ?>
						</small>

						<div class="lgp-form-group" style="margin-top:8px;">
							<label for="lgp-video-file"><?php esc_html_e('Upload Video (optional)', 'loungenie-portal'); ?></label>
							<input type="file" id="lgp-video-file" accept="video/mp4,video/quicktime,video/webm,video/ogg" class="lgp-input" />
						</div>
					</div>
