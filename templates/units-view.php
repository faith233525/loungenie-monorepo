...
// PERFORMANCE OPTIMIZATION: Cache unit list for 5 minutes per company/role
$cache_key = $is_support ? 'lgp_units_list_all' : 'lgp_units_list_company_' . $company_id;
$units     = get_transient($cache_key);
...
if (false === $units) {
	...
	if (strtolower((string) $units_table_exists) === strtolower($units_table)) {
		...
		if (null === $units) {
			error_log(sprintf('LounGenie Portal: Units query failed in %s:%d - %s', __FILE__, __LINE__, $wpdb->last_error));
			$units = array();
		}
	} else {
		$units = array();
	...
	// Cache for 5 minutes
	set_transient($cache_key, $units, 5 * MINUTE_IN_SECONDS);
}
					...
					<tbody>
						<?php foreach ($units as $unit) : ?>
							<tr data-color="<?php echo esc_attr($unit->color_tag ?? ''); ?>"
								...
								<td>
									<?php if ($unit->season) : ?>
										<span class="lgp-badge lgp-badge-<?php echo esc_attr('seasonal' === $unit->season ? 'warning' : 'info'); ?>">
