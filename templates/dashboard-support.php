...
// Color hex helper provided by LGP_Company_Colors::lgp_get_color_hex() via LGP_Loader
...
// Recent tickets - PERFORMANCE: Cache for 2 minutes
$recent_tickets_cache_key = 'lgp_dashboard_recent_tickets';
$recent_tickets           = get_transient($recent_tickets_cache_key);
...
if (false === $recent_tickets) {
	...
	set_transient($recent_tickets_cache_key, $recent_tickets, 2 * MINUTE_IN_SECONDS);
}

// Top 5 Metrics - PERFORMANCE: Cache all aggregation queries for 10 minutes
$top_metrics_cache_key = 'lgp_dashboard_top_metrics';
$top_metrics_data      = get_transient($top_metrics_cache_key);

if (false === $top_metrics_data) {
	...
	// Cache all top metrics for 10 minutes
	$top_metrics_data = compact('top_colors', 'top_lock_brands', 'top_venues', 'seasonal_units', 'yearround_units');
	set_transient($top_metrics_cache_key, $top_metrics_data, 10 * MINUTE_IN_SECONDS);
} else {
	extract($top_metrics_data);
}
