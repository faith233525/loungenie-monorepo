...
error_reporting(0);
$basePath = dirname(__DIR__); // Goes up to /workspaces/Pool-Safe-Portal
$tests = [];
...
$tests[] = ['name' => 'All 11 templates present (consolidated)', 'result' => count(glob("$basePath/templates/*.php")) >= 11];
...
$tests[] = ['name' => 'No hardcoded credentials', 'result' => (function() use ($basePath) { $count = 0; foreach (glob("$basePath/includes/class-*.php") as $f) if (stripos(file_get_contents($f), "example.com") !== false) $count++; return $count === 0; })()];
$tests[] = ['name' => 'Nonce/CSRF protection (20+ uses)', 'result' => (function() use ($basePath) { $count = 0; foreach (glob("$basePath/api/*.php") as $f) $count += substr_count(file_get_contents($f), 'nonce'); return $count >= 20; })()];
$tests[] = ['name' => 'SQL injection prevention (20+ prepared)', 'result' => (function() use ($basePath) { $count = 0; foreach (glob("$basePath/api/*.php") as $f) $count += substr_count(file_get_contents($f), 'wpdb->prepare'); return $count >= 20; })()];
$tests[] = ['name' => 'Output properly escaped (50+ calls)', 'result' => (function() use ($basePath) { $count = 0; foreach (glob("$basePath/templates/*.php") as $f) $count += substr_count(file_get_contents($f), 'esc_'); return $count >= 50; })()];
...
$tests[] = ['name' => 'CSS properly scoped (no global)', 'result' => (function() use ($basePath) { $unscoped = 0; foreach (glob("$basePath/assets/css/*.css") as $f) { $c = file_get_contents($f); $n = basename($f); if (!preg_match('/(tokens|variables|reset|login-page)/i', $n) && preg_match('/^\s*(body|html|form)\s*{/m', $c) && !preg_match('/\.lgp-portal/m', $c)) $unscoped++; } return $unscoped === 0; })()];
