...
class LGP_Notifications {
    ...
    public static function notify_ticket_event( $ticket, $event, $priority = 'medium' ) {
        ...
        if ( class_exists( 'LGP_Logger' ) ) {
            LGP_Logger::log_notification( $support_to, 'email', $priority, $ticket_id, $company_id, array( 'role' => 'support', 'event' => $event ) );
        }
        ...
        if ( $partner_to && $partner_uid ) {
            ...
            if ( class_exists( 'LGP_Logger' ) ) {
                LGP_Logger::log_notification( $partner_uid, 'email', $priority, $ticket_id, $company_id, array( 'role' => 'partner', 'event' => $event ) );
                LGP_Logger::log_notification( $partner_uid, 'portal', $priority, $ticket_id, $company_id, array( 'role' => 'partner', 'event' => $event ) );
            }
        }
    }
}
