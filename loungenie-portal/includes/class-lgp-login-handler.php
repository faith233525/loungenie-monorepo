...
class Login_Handler {
    ...
    public static function init() {
        ...
        add_action('init', [__CLASS__, 'register_login_page']);
        
        // Handle form submissions
        add_action('init', [__CLASS__, 'handle_partner_login']);
        add_action('init', [__CLASS__, 'handle_sso_login']);
        
        // Hook into WordPress login redirection
        add_filter('login_url', [__CLASS__, 'custom_login_url']);
        add_filter('wp_login_url', [__CLASS__, 'custom_login_url']);
        
        // Redirect to custom login page
        add_action('login_init', [__CLASS__, 'redirect_to_custom_login']);
        
        // Handle logout
        add_action('wp_logout', [__CLASS__, 'handle_logout']);
        
        // Add login page body class
        add_filter('body_class', [__CLASS__, 'add_login_body_class']);
        
        // Add login redirect based on role
        add_filter('login_redirect', [__CLASS__, 'redirect_on_role'], 10, 3);
    }
...
add_action('plugins_loaded', ['\LounGenie\Portal\Login_Handler', 'init'], 5);
