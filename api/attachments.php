...
/**
 * REST endpoints for ticket attachments.
 */
class LGP_Attachments_API {
	 ...
	 * @param int   $user_id User uploading the file.
	...
	private static function handle_file_upload( $file, $ticket_id, $user_id ) {
		...
		if ( ! file_exists( $htaccess_file ) ) {
			...
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_put_contents_file_system
	...
	public static function delete_attachment( $request ) {
		...
		$is_uploader = (int) $attachment->uploaded_by === (int) $current_user_id;
	...
	public static function download_attachment( $request ) {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.readfile_file_get_contents
		readfile( $file_path );
		exit;
	}
}
