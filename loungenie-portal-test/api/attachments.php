...
class LGP_Attachments_API
{
	...
	public static function register_routes()
	{
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array(__CLASS__, 'check_ticket_access_with_nonce'),
				// Security: File upload requires CSRF protection
			)
		);
		...
		register_rest_route(
			...
			array(
				...
				'permission_callback' => array(__CLASS__, 'check_attachment_permission_with_nonce'),
	...
	public static function upload_attachment($request)
	{
		...
		foreach ($files as $file_key => $file) {
			...
			// Security: Validate file type using both MIME type and file signature (magic bytes)
			$validation = self::validate_file_security($file);
			if (! $validation['valid']) {
				$uploaded[] = array(
					'success' => false,
	...
	/**
	 * Validate file security with MIME type and file signature checking
	 * Security: Prevents malicious file uploads by validating both MIME type and magic bytes
	 * 
	 * @param array $file File array from $_FILES
	 * @return array Validation result with 'valid' boolean and 'error' message
	 */
	private static function validate_file_security($file)
	{
		// First check declared MIME type
		if (! in_array($file['type'], self::ALLOWED_TYPES, true)) {
			return array(
				'valid' => false,
				'error' => sprintf('File type %s not allowed. Allowed types: JPG, PNG, PDF, TXT, DOC, DOCX', $file['type']),
			);
		}

		// Security: Verify actual file content matches declared MIME type using file signature (magic bytes)
		if (function_exists('finfo_open')) {
			$finfo        = finfo_open(FILEINFO_MIME_TYPE);
			$detected_mime = finfo_file($finfo, $file['tmp_name']);
			finfo_close($finfo);

			// Verify detected MIME matches declared MIME or is in allowed list
			if ($detected_mime && ! in_array($detected_mime, self::ALLOWED_TYPES, true)) {
				return array(
					'valid' => false,
					'error' => 'File content does not match allowed file types (possible malicious file)',
				);
			}
		}

		// Security: Check for dangerous file extensions regardless of MIME type
		$filename  = strtolower($file['name']);
		$dangerous = array('.php', '.phtml', '.php3', '.php4', '.php5', '.exe', '.com', '.bat', '.cmd', '.sh', '.cgi');
	 ...
	 * Security: Files are stored with randomized names and protected by .htaccess
	 */
	private static function handle_file_upload($file, $ticket_id, $user_id)
	{
		...
		// Security: Generate unique filename to prevent path traversal and file overwrites
	...
	/**
	 * Check ticket access with CSRF protection for state-changing operations
	 * Security: Validates WordPress REST API nonce to prevent CSRF attacks
	 */
	public static function check_ticket_access_with_nonce($request)
	{
		// First check if user has ticket access
		if (! self::check_ticket_access($request)) {
			return false;
		}

		// Verify nonce for state-changing operations (file uploads)
	...
	/**
	 * Check attachment permission with CSRF protection
	 * Security: Validates WordPress REST API nonce to prevent CSRF attacks
	 */
	public static function check_attachment_permission_with_nonce($request)
	{
		// First check if user has attachment access
		if (! self::check_attachment_permission($request)) {
			return false;
		}
