...
class LGP_Loader
{
	...
	public static function init()
	{
		...
		// PERFORMANCE OPTIMIZATION: Initialize cache invalidation system
		self::maybe_init_class('LGP_Cache_Invalidation'); // Automatic cache clearing
		self::maybe_init_class('LGP_Geocode');           // Background geocoding queue

		self::register_rest_apis();
		self::maybe_init_class('LGP_i18n');         // Internationalization
		self::maybe_init_class('LGP_API_Docs');     // API Documentation
		...
		self::maybe_init_class('LGP_CSV_Company_Location_Import'); // CSV Company + Location + Lock Import
		self::maybe_init_class('LGP_Query_Monitor'); // Query Performance Monitoring

		// Admin Tools
		if (is_admin() || current_user_can('manage_options')) {
			$role_switcher_file = plugin_dir_path(__FILE__) . 'class-lgp-role-switcher.php';
			if (file_exists($role_switcher_file)) {
				require_once $role_switcher_file;
			} else {
