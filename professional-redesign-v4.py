...
HEADER_TEMPLATE_RAW = """<!-- wp:group {\"align\":\"full\",\"style\":{\"spacing\":{\"padding\":{\"top\":\"14px\",\"bottom\":\"14px\",\"left\":\"0\",\"right\":\"0\"}}},\"backgroundColor\":\"base\",\"layout\":{\"type\":\"constrained\"}} -->
<div class=\"wp-block-group alignfull has-base-background-color has-background\" style=\"padding-top:14px;padding-right:0;padding-bottom:14px;padding-left:0\">
  ...
  <div class=\"wp-block-group alignwide\">
    ...
    <div class=\"wp-block-group\">
      <!-- wp:site-logo {\"width\":170,\"shouldSyncIcon\":true} /-->
    </div>
    <!-- /wp:group -->
...
.wp-site-blocks > header .current-menu-item > .wp-block-navigation-item__content,
.wp-site-blocks > header .current_page_item > .wp-block-navigation-item__content {
  color: #0055a5 !important;
  background: #eaf4ff !important;
}

/* Professional icon style replacing emoji symbols */
.lg .lg-icon {
  width: 1.1em;
  height: 1.1em;
...
.lg .i-award{mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M7 2h10v3a4 4 0 0 1-3 3.87V11h2v2h-2.2l1.2 7h-2l-1-6h-0l-1 6H9l1.2-7H8v-2h2V8.87A4 4 0 0 1 7 5V2zM9 4v1a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2V4H9z"/></svg>');-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M7 2h10v3a4 4 0 0 1-3 3.87V11h2v2h-2.2l1.2 7h-2l-1-6h-0l-1 6H9l1.2-7H8v-2h2V8.87A4 4 0 0 1 7 5V2zM9 4v1a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2V4H9z"/></svg>')}
.lg .i-calendar{mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M7 2h2v2h6V2h2v2h3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h3V2zm13 8H4v10h16V10z"/></svg>');-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M7 2h2v2h6V2h2v2h3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h3V2zm13 8H4v10h16V10z"/></svg>')}
.lg .i-revenue{mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M3 17l6-6 4 4 7-8v4h2V3h-8v2h4l-5 6-4-4-7 7z"/></svg>');-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="black" d="M3 17l6-6 4 4 7-8v4h2V3h-8v2h4l-5 6-4-4-7 7z"/></svg>')}

/* ═══════════════════════════════════════════════════
   LAYOUT
   ═══════════════════════════════════════════════════ */
.lg { color: #1a2440; overflow-x: hidden; }
.lg .wrap    { max-width: 1320px; margin: 0 auto; padding: 0 40px; }
.lg .wrap-sm { max-width: 900px;  margin: 0 auto; padding: 0 40px; }
...
def update_page(page_id, title, html, template="page-wide"):
    html = normalize_html_for_production(html)
    wrapped = "<!-- wp:html -->\n" + html + "\n<!-- /wp:html -->"
    payload = json.dumps({
        "title": title,
        "content": wrapped,
...
def normalize_html_for_production(html):
    # Remove emoji entities and use neutral icon spans for a professional look.
    replacements = {
        "&#x1f3c6;": '<span class="lg-icon i-award" aria-hidden="true"></span>',
        "&#x1f4c5;": '<span class="lg-icon i-calendar" aria-hidden="true"></span>',
        "&#x1f4c8;": '<span class="lg-icon i-revenue" aria-hidden="true"></span>',
        "&#x25b6;": "▶",
    }
    out = html
    for old, new in replacements.items():
      out = out.replace(old, new)
    # Strip any remaining emoji entities we did not map (mostly old decorative icons).
    out = re.sub(r"&#x1f[0-9a-f]+;", "", out, flags=re.IGNORECASE)
    out = re.sub(r"&#x26a1;", "", out, flags=re.IGNORECASE)
    out = re.sub(r"&#x2614;", "", out, flags=re.IGNORECASE)
    return out

def update_navigation(nav_id=4):
    payload = json.dumps({"content": NAVIGATION_RAW, "status": "publish"}).encode()
    req = urllib.request.Request(
        f"{NAVS}/{nav_id}", data=payload, method="POST",
        headers={**HEADERS, "Content-Length": str(len(payload))}
    )
    try:
...
def update_header_template():
    payload = json.dumps({"content": HEADER_TEMPLATE_RAW, "status": "publish"}).encode()
    req = urllib.request.Request(
        f"{PARTS}/twentytwentyfour//header", data=payload, method="POST",
        headers={**HEADERS, "Content-Length": str(len(payload))}
    )
    try:
        with urllib.request.urlopen(req, timeout=30):
            return True, "header template updated"
    except urllib.error.HTTPError as e:
        return False, f"HTTP {e.code}: {e.read().decode()[:300]}"
    except Exception as e:
        return False, str(e)
...
# ════════════════════════════════════════════════════════════════════════════
print("=" * 64)
print("LounGenie Professional Redesign v4  |  TT4 Full-Width + Modern Header")
print("=" * 64)

print("\n[1/4] Fixing nav menu labels...")
...
print("\n[2/4] Rebuilding FSE header and navigation...")
ok_nav, msg_nav = update_navigation(4)
ok_head, msg_head = update_header_template()
print(f"  {'✓' if ok_nav else '✗'} Navigation 4: {msg_nav}")
print(f"  {'✓' if ok_head else '✗'} Template-part header: {msg_head}")

print("\n[3/4] Publishing pages (template: page-wide)...")
pages = [
...
Changes in v4:
  ...
  • Header template rebuilt: larger logo, no duplicate site title, cleaner nav
  • Navigation source rebuilt: short labels and only key pages
  • Emoji entities replaced with cleaner icon treatment in-page
  ...
  • Wider layout containers for a more modern desktop feel
