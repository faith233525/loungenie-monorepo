...
class LGP_Outlook {
	...
	private static function get_redirect_uri() {
		$mode = get_option( 'lgp_outlook_redirect_mode', 'admin' );
		switch ( $mode ) {
			case 'ajax':
	...
	public static function render_settings_page() {
		...
		$redirects       = array(
			'admin' => admin_url( 'options-general.php?page=lgp-outlook-settings&oauth_callback=1' ),
			'ajax'  => admin_url( 'admin-ajax.php?action=lgp_outlook_oauth_callback' ),
			'front' => home_url( '/' . self::FRONT_CALLBACK_PATH ),
		);
		// Normalize schemes if site is HTTPS
		$home_scheme = parse_url( home_url(), PHP_URL_SCHEME );
		if ( 'https' === $home_scheme && function_exists( 'set_url_scheme' ) ) {
			foreach ( $redirects as $k => $u ) {
				$redirects[ $k ] = set_url_scheme( $u, 'https' );
			}
		}
		?>
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<tr>
						<th scope="row">
							<label for="lgp_outlook_redirect_mode"><?php esc_html_e( 'Redirect URI Mode', 'loungenie-portal' ); ?></label>
						</th>
						<td>
							<select id="lgp_outlook_redirect_mode" name="lgp_outlook_redirect_mode">
								<option value="admin" <?php selected( $redirect_mode, 'admin' ); ?>><?php esc_html_e( 'Admin Settings Page (default)', 'loungenie-portal' ); ?></option>
								<option value="ajax" <?php selected( $redirect_mode, 'ajax' ); ?>><?php esc_html_e( 'Admin Ajax Endpoint', 'loungenie-portal' ); ?></option>
								<option value="front" <?php selected( $redirect_mode, 'front' ); ?>><?php esc_html_e( 'Pretty URL (/psp-azure-callback)', 'loungenie-portal' ); ?></option>
							</select>

							<p class="description">
								<?php esc_html_e( 'Pick one registered in Azure. You can register all three to keep flexibility across environments.', 'loungenie-portal' ); ?>
							</p>
						</td>
					</tr>
				</table>
			...
			<ol>
				...
				<li><?php esc_html_e( 'Register the following Redirect URIs (at least one):', 'loungenie-portal' ); ?>
					<ul>
						<li><code><?php echo esc_html( $redirects['admin'] ); ?></code> — <?php esc_html_e( 'Admin Settings Page (default)', 'loungenie-portal' ); ?></li>
						<li><code><?php echo esc_html( $redirects['ajax'] ); ?></code> — <?php esc_html_e( 'Admin Ajax Endpoint', 'loungenie-portal' ); ?></li>
						<li><code><?php echo esc_html( $redirects['front'] ); ?></code> — <?php esc_html_e( 'Pretty URL (no auth required)', 'loungenie-portal' ); ?></li>
					</ul>
				</li>
