...
<body class="lgp-portal-body">
	...
	<header class="lgp-header">
		<div class="lgp-logo">
			...
			$site_icon    = $site_icon_id ? wp_get_attachment_image_src( $site_icon_id, 'full' ) : false;
			if ( $site_icon ) {
				echo '<img src="' . esc_url( $site_icon[0] ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" style="height:36px;width:auto;" />';
			} else {
		...
		<div class="lgp-header-actions">
			...
			<div class="lgp-user-menu">
				...
				<?php
				$login_return = home_url( '/portal' );
				$login_url    = wp_login_url( $login_return );
				$logout_url   = wp_logout_url( $login_url );
				?>
				<a href="<?php echo esc_url( $logout_url ); ?>" class="lgp-btn lgp-btn-secondary">
